...
import { ArrowLeft, Menu, Sliders, LayoutGrid, Monitor, Sparkles } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#fcf9f3] text-[#43372f] antialiased selection:bg-[#c8a978]/20 relative overflow-hidden flex flex-col h-screen">
      ...
      >
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          {/* Shadcn Sheet Modal for choosing templates */}
          <Sheet>
            ...
            {/* Slide-out Sheet Panel styled elegantly */}
            <SheetContent 
              side="left" 
      ...
      {/* Mobile Actions Overlay Drawer */}
      {menuOpen && (
        <div className="lg:hidden bg-white/95 backdrop-blur-lg border-b border-[#ece4d8] p-4 space-y-3 shadow-md relative z-50">
          <Sheet>
            <SheetTrigger asChild>
              <button className="w-full flex items-center gap-2 p-3 rounded-xl text-sm font-medium text-zinc-600 hover:bg-[#faf6f0] text-left">
                <LayoutGrid size={16} className="text-[#b99863]" />
                Change Template Layout
              </button>
            </SheetTrigger>
            <SheetContent side="bottom" className="h-[75vh] bg-white rounded-t-[32px] p-6 flex flex-col">
              <SheetHeader className="text-left border-b pb-4">
                <SheetTitle className="font-serif text-xl">Templates</SheetTitle>
              </SheetHeader>
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-8 flex flex-col items-stretch overflow-hidden relative z-10">
        ...
        <div className="flex items-center justify-between px-4 pb-3 flex-shrink-0">
          ...
          <span className="text-[10px] text-zinc-400 flex items-center gap-1">
            <Monitor size={10} /> Editorial Scaling Layout
          </span>
        </div>
